...
import * as protoHttpResolve from "@/actor/protocol/http/resolve";
...
import { ClientDriver } from "@/app/inline-client-driver";
...
export function createManagerRouter(
	...
	// POST /actors/resolve
	{
		...
		app.openapi(resolveRoute, (c) => handleResolveRequest(c, clientDriver));
	}
...
async function handleActionRequest(
	...
	try {
		...
		// Call action
		const output = await clientDriver.action(
			c.req,
			params.data.query,
			encoding,
			parameters,
...
async function handleResolveRequest(
	...
	const actorId = await clientDriver.resolveActorId(
		c.req,
		params.data.query,
		encoding,
	);
