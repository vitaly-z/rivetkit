...
describe("Key serialization and deserialization", () => {
  ...
  describe("serializeKey", () => {
    ...
    test("serializes multiple keys", () => {
      expect(serializeKey(["a", "b", "c"])).toBe(`a${KEY_SEPARATOR}b${KEY_SEPARATOR}c`);
    });
    ...
    test("handles complex keys", () => {
      expect(serializeKey(["a,b", EMPTY_KEY, "c,d"])).toBe(`a\\,b${KEY_SEPARATOR}\\${EMPTY_KEY}${KEY_SEPARATOR}c\\,d`);
    });
  });
  ...
  describe("deserializeKey", () => {
    ...
    test("deserializes multiple keys", () => {
      expect(deserializeKey(`a${KEY_SEPARATOR}b${KEY_SEPARATOR}c`)).toEqual(["a", "b", "c"]);
    });
    ...
    test("deserializes complex keys", () => {
      expect(deserializeKey(`a\\,b${KEY_SEPARATOR}\\${EMPTY_KEY}${KEY_SEPARATOR}c\\,d`)).toEqual(["a,b", EMPTY_KEY, "c,d"]);
    });
  });
  ...
  describe("serializeNameAndKey", () => {
    ...
    test("serializes name with multiple keys", () => {
      expect(serializeNameAndKey("test", ["a", "b", "c"])).toBe(`test:a${KEY_SEPARATOR}b${KEY_SEPARATOR}c`);
    });
