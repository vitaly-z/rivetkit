...
export async function waitForLobbyReady(
	...
	switch (status) {
		...
		case "ready":
			...
			return newLobby;
		case "aborted": {
			const destroyMeta = currentState(c).lobbyDestroyMeta[lobbyId];
...
export async function destroyLobby(
	c: LobbyManagerContext,
	req: { lobbyId: string; reason?: string; cause?: ActorError },
) {
	// Get lobby
	...
	currentState(c).lobbyDestroyMeta[req.lobbyId] = {
