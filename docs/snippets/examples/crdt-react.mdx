...
export function YjsEditor({ documentId = "shared-doc" }) {
  ...
  const { worker } = useWorker("yjsDocument", { 
    tags: { documentId }
  });
  ...
  useEffect(() => {
    ...
    yText.observe(() => {
      ...
      if (!updatingFromServer.current) {
        ...
        if (worker && !updatingFromLocal.current) {
          ...
          worker.applyUpdate(base64).finally(() => {
  ...
  useWorkerEvent({ worker, event: "initialState" }, ({ update }) => {
    const yDoc = yDocRef.current;
    if (!yDoc) return;
  ...
  useWorkerEvent({ worker, event: "update" }, ({ update }) => {
    const yDoc = yDocRef.current;
    if (!yDoc) return;
