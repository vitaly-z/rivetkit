...
export function OrgDashboard({ orgId }: { orgId: string }) {
  ...
  // Connect to tenant worker with authentication token
  const [{ worker }] = useWorker("tenant", { 
    params: { token },
    tags: { orgId }
  });

  // Load data when worker is available
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const membersList = await worker.getMembers();
        setMembers(membersList);
        
        // Try to get invoices (only available to admins)
        try {
          const invoicesList = await worker.getInvoices();
