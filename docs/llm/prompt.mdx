...
- **Action**: Method for a worker to expose callable functions to clients
- **Event**: Asynchronous message sent from a worker to connected clients
...
- **Cloudflare Workers**: Uses Durable Objects for worker state persistence
...
- Each worker owns and manages its own isolated state via `c.state`
...
- **Actions**: Used for clients to call worker functions
- **Events**: For workers to publish updates to clients
- Actions are defined in the `actions` object of the worker configuration
...
- Clients subscribe to events with `worker.on(eventName, callback)`

## Lifecycle Hooks

- `createState()`: Function that returns initial worker state
- `onStart(c)`: Called any time worker is started (after restart/upgrade)
- `onStateChange(c, newState)`: Called when worker state changes
...
- Keep state minimal and relevant to the worker's core function
- Use separate workers for different entity types (users, rooms, documents)
- Avoid too many cross-worker communications
...
- Scheduled events persist across worker restarts
...
- Prefer functional worker pattern with `worker({ ... })` syntax
