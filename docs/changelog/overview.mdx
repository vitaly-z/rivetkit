...
<Update label="0.7.0" description="March 16th, 2025">
  ...
  - **Shorthand State Initiation**: Use  `state` and `connectionState` to create state without having to implement `createState` and `createConnectionState`
  ...
  - Add `createConnectionState` instead of using return value from `onBeforeConnect` for clarity
