...
Called only once when the actor is first created. This method returns the initial state of the actor (see [state documentation](/concepts/state)).
...
```typescript
import { Actor, type Rpc, OnBeforeConnectOptions } from "actor-core";
