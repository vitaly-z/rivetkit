...
applications or for storing relational data. Read more [here](/actors/external-sql).
</Tip>
...
The `createState` function is called once when the actor is first created. See [Lifecycle](/actors/lifecycle) for more details.
...
To interact with a actor's state, you must use [Actions](/actors/actions). Actions provide a controlled way to read from and write to the state.
...
2. Use an external database, see [External SQL Databases](/actors/external-sql)
