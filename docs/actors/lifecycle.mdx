---
title: Lifecycle
description: Understand actor lifecycle hooks and initialization patterns
icon: rotate
---
...
The `createState` function or `state` constant defines the initial state of the actor (see [state documentation](/actors/state)). The `createState` function is called only once when the actor is first created.

### `createVars` and `vars`

The `createVars` function or `vars` constant defines ephemeral variables for the actor (see [state documentation](/actors/state)). These variables are not persisted and are useful for storing runtime-only objects or temporary data.
...
See [Helper Types](/actors/helper-types) for more details on using `ActorContextOf`.
