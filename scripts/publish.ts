...
async function main() {
  ...
  // Commented out git steps preserved from original
...
async function getPublicPackages() {
  ...
  const { stdout: packagesStdout } = await $`yarn workspaces list --json --no-private`;
...
function validatePackages(publicPackages: any[]) {
  const nonActorCorePackages = publicPackages.filter(pkg => 
    pkg.name !== "actor-core" && !pkg.name.startsWith("@actor-core/")
  );
  
  if (nonActorCorePackages.length > 0) {
    console.error(chalk.red("Error: Found non-actor-core packages in public packages:"));
    for (const pkg of nonActorCorePackages) {
      console.error(chalk.red(`  - ${pkg.name} (${pkg.location})`));
    }
    console.error(chalk.red("Please ensure these packages are marked as private or have correct naming."));
    process.exit(1);
  }
  
  console.log(chalk.blue(`Found ${publicPackages.length} actor-core packages to publish`));
}
...
async function publishPackages(publicPackages: any[], version: string) {
  ...
  for (const pkg of publicPackages) {
    await publishPackage(pkg, rootDir);
  }

  console.log(chalk.green(`✅ Published all packages at version ${version}`));
  console.log(chalk.yellow("⚠️ Make sure to merge Release Please"));
}

async function publishPackage(pkg: any, rootDir: string) {
  const { location, name } = pkg;
  const fullPath = path.join(rootDir, location);
  
  try {
    console.log(chalk.cyan(`Publishing ${name} from ${location}...`));
    
    // Change to package directory
    process.chdir(fullPath);
    
    // Run npm publish
    await $`npm publish --access public --tolerate-republish`;
