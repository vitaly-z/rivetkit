...
export function DocumentEditor() {
	...
	const documentId =
		new URLSearchParams(window.location.search).get("id") || "default-doc";
	const [{ actor, connectionId }] = useActor("document", {
		tags: { id: documentId },
	});
	...
	useActorEvent(
		{ actor, event: "textUpdated" },
		({ text: newText, userId: senderId }) => {
			if (senderId !== connectionId) setText(newText);
		},
	);

	useActorEvent(
		{ actor, event: "cursorUpdated" },
		({ userId: cursorUserId, x, y }) => {
